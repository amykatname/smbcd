...
--		else
--			loadbackground("9-2.txt")
--		end
	else
--		if sixteenbynine then
			loadbackground("9-1.txt")
--		else
--			loadbackground("9-2.txt") --not needed anymore, logo is standalone
--		end
...
function menu_draw()
	...
	if gamestate == "menu" then
		...
		elseif selection == 1 then
			love.graphics.draw(menuselectimg, (funnyoffset-(math.ceil((utf8.len(TEXT["player game"])+2)/2)*8)-14)*scale, (137+(selection-1)*16)*scale, 0, scale, scale)
		elseif selection == 2 then
			love.graphics.draw(menuselectimg, (funnyoffset-(math.ceil((utf8.len(TEXT["options"]))/2)*8)-16)*scale, (137+(selection-1)*16)*scale, 0, scale, scale)
		...
		for i = start, 9 do
			...
			if continueavailable then
				...
				properprintfunc(TEXT["continue game"], (funnyoffset-(math.ceil(utf8.len(TEXT["continue game"])/2)*8))*scale, 122*scale)
			end
			...
			properprintfunc(TEXT["player game"], (funnyoffset-(math.ceil((utf8.len(TEXT["player game"])+2)/2)*8)+16)*scale, 138*scale)
			
			properprintfunc(players, (funnyoffset-(math.ceil((utf8.len(TEXT["player game"])+2)/2)*8))*scale, 138*scale)
			...
			properprintfunc(TEXT["options"], (funnyoffset-(math.ceil(utf8.len(TEXT["options"])/2)*8))*scale, 154*scale)
			...
			--properprintfunc(TEXT["level editor"], (143-(math.ceil(utf8.len(TEXT["level editor"])/2)*8))*scale, 170*scale)
			...
			--properprintfunc(TEXT["select mappack"], (143-(math.ceil(utf8.len(TEXT["select mappack"])/2)*8))*scale, 178*scale)
	...
	elseif gamestate == "options" then
		...
		elseif optionstab == 2 then
			...
			if v.portalgununderhat then
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(v["animations"][0], v["small"]["idle"][3], (108+v.smalloffsetX*2)*scale, (32+32-v.smalloffsetY*2)*scale, 0, scale*2, scale*2, v.smallquadcenterX, v.smallquadcenterY)
			end
			love.graphics.setColor(255, 255, 255, 255)
			for i = 1, #v.colorables do
				...
				if mariocharacter[skinningplayer] then
					love.graphics.draw(v["animations"][i], v["small"]["idle"][3], (108+v.smalloffsetX*2)*scale, (32+32-v.smalloffsetY*2)*scale, 0, scale*2, scale*2, v.smallquadcenterX, v.smallquadcenterY)
				end
			end
			...
			properprintF("{", 64*scale+100, 56*scale)
			properprintF("}", 112*scale+100, 56*scale)
	...
	--		love.graphics.draw(huebarmarkerimg, math.floor(28 + (portalhues[skinningplayer][2])*178)*scale, 190*scale, 0, scale, scale)
		elseif optionstab == 3 then
			...
			properprintF(TEXT["aspect ratio:"], 30*scale, 50*scale)
			if sixteenbynine then
				properprintF(TEXT["4:3"], (180-utf8.len(TEXT["4:3"])*8)*scale, 50*scale)
				--if width == 25 then
				--width = 16
				--changescale(scale, fullscreen)
				--end
			else
				properprintF(TEXT["16:9"], (180-utf8.len(TEXT["16:9"])*8)*scale, 50*scale)
				--if width == 16 then
				--width = 25
				--changescale(scale, fullscreen)
				--end
...
--		end
--		--level editor
--		if x > (143-(math.ceil(utf8.len(TEXT["options"])/2)*8)) and y > 154 and x < (143+(math.ceil(utf8.len(TEXT["options"])/2)*8)) and y < 154+8 then
--			selection = 2
--			mouseonselect = 2
--		end
--		--select mappack
--		if x > (143-(math.ceil(utf8.len(TEXT["level editor"])/2)*8)) and y > 170 and x < (143+(math.ceil(utf8.len(TEXT["level editor"])/2)*8)) and y < 170+8 then
--			selection = 3
--			mouseonselect = 3
--		end
--		--options
--		if x > (143-(math.ceil(utf8.len(TEXT["select mappack"])/2)*8)) and y > 186 and x < (143+(math.ceil(utf8.len(TEXT["select mappack"])/2)*8)) and y < 186+8 then
--			selection = 4
--			mouseonselect = 4
--		end
	end
